...
export class DatabaseService {
  ...
  private async checkSupabaseConnection(): Promise<void> {
    try {
      ...
      if (error) {
        console.log('[DB] Supabase connection test failed - using localStorage only');
        this.isSupabaseAvailable = false;
      } else {
  ...
  async saveReport(report: SEOReport): Promise<void> {
    ...
    try {
      const { error } = await safeSupabaseOperation(() =>
        supabase!.from('seo_reports').upsert({
          id: report.id,
  ...
  async saveTrackingCode(code: TrackingCode): Promise<void> {
    ...
    try {
      const { error } = await safeSupabaseOperation(() =>
        supabase!.from('tracking_codes').upsert({
          id: code.id,
          user_id: code.userId,
